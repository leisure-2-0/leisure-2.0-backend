...
class UpcomingFestivalQueryServiceTest {
    ...
    void regionStripRules() {
        given(festivalRepository.findUpcomingFestivals(any())).willReturn(List.of(
                result("강릉시"),          // 3글자 → 강릉
                result("남양주시"),        // 4글자 → 남양주
                result("종로구"),          // 3글자 → 종로
                result("남구"),            // 2글자 → 유지
        ...
        List<String> regions = festivalQueryService.getUpcomingFestivals().stream()
    ...
    void passthrough() {
        ...
        UpcomingFestivalResponse r = festivalQueryService.getUpcomingFestivals().get(0);
