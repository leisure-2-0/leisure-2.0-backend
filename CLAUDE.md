...
- **프로필 이미지 정책** — **회원가입에선 프로필 이미지를 받지 않는다**(`SignUpRequest`에 필드 없음, `Member.create`도 파라미터 없이 항상 `profileImageUrl=null` 생성). 이미지 **최초 저장은 마이페이지 프로필 수정**(`MemberService.changeProfile`)에서 이뤄진다 — null=유지 / 빈문자열·공백=제거(null 저장) / 값=저장(**최초 저장 포함**). 서비스가 `profileImageUrl != null`일 때만 `Member.changeProfileImageUrl`(blank→null, else trim) 호출. 실제 업로드(PreSigned URL 등)는 프로필 수정 경로에 붙일 TODO. (`Member.normalizeProfileImageUrl`은 가입에서 이미지 제거되며 미사용이 돼 **주석 처리된 레거시**.)
...
`com.leisure.pointhistory`. 게시/좋아요/북마크 행위가 **작성자에게 여가 포인트를 적립**한다. 도메인 행위와 적립을 **Spring 이벤트(인프로세스) → RabbitMQ(프로세스 간)** 2단으로 비동기 분리한다. 설계 상세(문제정의·트레이드오프·폐기한 대안·코드)는 별도 "포인트 파이프라인 설계" 문서 참고.

- **점수(`PointType`)** — `POST_PUBLISH`=5 / `LIKE_RECEIVED`=2 / `BOOKMARK_RECEIVED`=2 (enum이 amount 보유).
- **적립 정책(잠금)** — ① 1인당 1회(`unique(actor_id, source_id, point_type)`), ② 취소 시 차감 없음(원장 append-only), ③ 토글 재적립 없음(중복키 조용히 무시), ④ **self 제외**(좋아요/북마크는 본인이 자기 글에 누르면 적립 X, **게시는 본인이라도 적립**), ⑤ 글 삭제→포인트 유지, 회원탈퇴→소프트삭제+랭킹 제외(30일 배치 퍼지 시 `member_id`(수령자) 기준만 삭제, `actor_id` 기준은 타인 포인트라 유지).
- **흐름 2단** — 서비스는 `ApplicationEventPublisher.publishEvent(PostXxxEvent)` **한 줄**만(point/봉투/RabbitMQ 모름). `PointEarnEventListener`(`pointhistory.listener`)가 `@TransactionalEventListener(AFTER_COMMIT)`로 받아 **self 제외 판정 + `EventEnvelope<PointEarnMessage>` 조립 + `DomainEventPublisher.publish`**. 커밋 성공 후에만 발행돼 롤백 시 유령 포인트가 없다. self 제외·봉투·eventId·PointType 매핑이 **리스너 한 곳**에 집결(서비스는 자기 도메인 이벤트만 발행 → post/like/bookmark가 point를 모름). ⚠️ 서비스가 주입받는 `eventPublisher`는 Spring 기본 `ApplicationEventPublisher`(인프로세스)로, RabbitMQ 발행자 `DomainEventPublisher`와 이름이 비슷하나 별개다.
- **도메인 이벤트 record** — `PostPublishedEvent`(post.event)/`PostLikedEvent`(postlike.event)/`PostBookmarkedEvent`(bookmark.event). **id(Long)만** 담는다 — `open-in-view=false`라 AFTER_COMMIT 시점엔 트랜잭션이 닫혀 엔티티/지연연관을 담으면 위험. (`member.event.MemberWithdrawnEvent`와 동일 패턴 — 도메인 이벤트는 발행 도메인이 소유, 봉투/발행자만 `global.event`.)
- **멱등 적립(`PointHistoryService.earn`)** — `point_histories`에 **`INSERT IGNORE`**(MySQL, Postgres `ON CONFLICT` 아님) → 삽입된 경우만 `MemberRepository.addPoint`(원자적 `@Modifying` UPDATE, `deletedAt is null`). 0행이면 이미 적립(조용히 종료) → at-least-once 재전송에도 중복 적립 없음. addPoint 0행이면 수령자 없음/탈퇴 → `MEMBER_NOT_FOUND`로 롤백. **원장(`PointHistory`)은 `BaseCreatedEntity`(수정/삭제 없는 append-only)**, `amount`는 적립 시점 정책 점수를 보존.
- **RabbitMQ 인프라(`global.event`, `global.config.RabbitMqConfiguration`)** — 도메인 무관 범용 발행/봉투. `EventEnvelope<T>`(`global.event.dto`) = payload(`data`) + 메타(eventId/eventType/correlationId/occurredAt/version). **`eventType`(봉투=라우팅)과 `PointType`(payload=점수)은 계층이 달라 분리** — 봉투에 도메인 enum을 넣으면 범용 인프라가 특정 도메인에 의존(역전)하고 payload를 안 열고 라우팅하는 원칙이 깨진다. eventId는 `global.utils.EventIdGenerator.generate()`(UUID), 추적 인프라 전까진 correlationId로도 재사용.
  - **토폴로지** — topic exchange `domain.events` + DLX `domain.events.dlx`, 큐 `api.domain.events.queue`(arg `x-dead-letter-exchange`→DLX) + DLQ `api.domain.events.dlq`. 이름은 `application.yml`의 `rabbitmq:`→`RabbitMqProperties`. **라우팅 키** = `EventType`→`post.{published,liked,bookmarked}.pointed`(capability 태그 `.pointed`), 큐 바인딩 `#.pointed`, DLQ 바인딩 `#`. ⚠️ `DomainEventPublisher.from(eventType, ".pointed")`이 접미사를 붙여 **현재는 point 전용** — 알림/통계 등 두 번째 소비자 추가 시 capability를 이벤트별로 결정하도록 탈하드코딩 필요.
  - **발행 신뢰성** — publisher confirm(`correlated`, per-message `CorrelationData` future로 ack/nack/timeout 로그, eventId를 상관키로) + returns(`mandatory: true`, 라우팅 실패 감지 — confirm이 대체 못 하는 별개 신호). **전역 `setConfirmCallback`은 per-message future와 충돌하므로 주석 처리**됨(returns 콜백은 유지).
  - **소비(`PointEarnConsumer`, `pointhistory.consumer`)** — `EventEnvelope<PointEarnMessage>` 수신 후 `.data()` 언래핑. **MANUAL ack**: 성공 `basicAck` / 예외 catch해 `basicNack(requeue=false)`→DLQ. `default-requeue-rejected: false` + retry off로 **첫 실패 즉시 격리**(무한 재전송 루프 방지). ⚠️ **선언적 재시도(`spring.rabbitmq.listener.simple.retry.enabled=true`)는 이 MANUAL+직접 nack 구조에선 동작하지 않는다**(리스너가 예외를 삼켜 retry 인터셉터가 못 받음) — 재시도가 필요하면 **AUTO ack + 예외 전파**로 바꿔야 한다. earn이 멱등이라 재시도 자체는 안전. (관측성 로그는 `[point-publisher]`/`[point-consumer]` 태그 + eventId/correlationId로 발행~소비 관통.)
- **[레거시] `DomainEventPublisher.publishAfterCommit`** — 서비스가 발행자를 직접 부르던 시절 커밋 후 발행을 보장하려던 트랜잭션 동기화 훅(`TransactionSynchronization.afterCommit`). 이벤트 + `@TransactionalEventListener(AFTER_COMMIT)` 방식이 이미 커밋 후 지점이라 **중복 → 주석 처리**(참고용, 지우지 말 것). 리스너는 raw `publish`를 부른다.
- **스키마** — Flyway `V4__add_point_history_and_member_point.sql`(`point_histories` 테이블 + `members.point` default 0). ⚠️ `point_type`을 `varchar(30)`으로 뒀는데 Hibernate가 `@Enumerated(STRING)`을 MySQL `enum(...)`으로 볼 수 있어(V1의 status/category가 그렇게 생성됨) 첫 기동 `validate` 통과 확인 필요 — 실패 시 `enum('BOOKMARK_RECEIVED','LIKE_RECEIVED','POST_PUBLISH')`(알파벳순)로 교체. `members.role`(varchar)과 동일 이슈.
- **테스트** — `PointHistoryServiceTest`(earn 멱등 3케이스), `PointEarnEventListenerTest`(self 제외 정책·봉투 필드 매핑·eventId=correlationId), `PointEarnConsumerTest`(성공 ack / 실패 nack→DLQ·언래핑), `DomainEventPublisherTest`(라우팅 키 변환·null/blank 검증). 전부 Mockito 순수 유닛(`@Tag` 없음). **실제 브로커 왕복(제네릭 `EventEnvelope` 역직렬화가 `.data()`로 실제 풀리는지)·Flyway V4 `validate`는 미검증** — `@Tag("integration")` + Testcontainers(RabbitMQ/MySQL) 도입 시 커버.
