...
`com.leisure.festival`. **한국관광공사 TourAPI 연동 도메인. 목록 배치 구현·검증 완료(651건, 멱등), 상세 보강 배치(소개글·홈페이지/운영시간)도 구현(실적재는 쿼터 리셋 후).** 클라이언트는 `com.leisure.global.external.tourapi`(RestClient), 도메인은 `domain`/`dto`/`dto.result`/`repository`/`service`/`controller`/`scheduler` 레이어를 갖춘다. 배치 설계 상세(쿼터 전략·트레이드오프·회고)는 별도 "축제 배치 설계" 문서 참고.
...
- **미구현(향후)** — **Festival↔Region 조인 조회 API**(두 ldong 코드로 LEFT JOIN해 regnName/signguName 프로젝션, QueryDSL — 축제엔 지명 미저장, Region이 단일 소스). 그리고 **재시도 정책 세분화**(현재 TourApiException만 빼고 다 재시도 → 429/4xx는 즉시 실패로).
